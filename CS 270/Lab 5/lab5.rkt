...
;---------------------------------------------------------------------------------------------------------
    [(implication? expr) (and (boolean-expr? (op1 expr))
                              (boolean-expr? (op2 expr)))] ; insert case for implication
    ; implication needs to boolean-expr operands
;---------------------------------------------------------------------------------------------------------
...
;---------------------------------------------------------------------------------------------------------
    ...
    ; p -> q == (not p) or q
    [(implication? expr) (or (not (boolean-eval (op1 expr) env))
                              (boolean-eval (op2 expr) env))]
;---------------------------------------------------------------------------------------------------------
...
(define (is-reserved-word? word)
  (cond
   ...
   [ (eq? word 'not) #t]
;---------------------------------------------------------------------------------------------------------
   [ (eq? word 'implies) #t] ; implies key word is reserved for implication
;---------------------------------------------------------------------------------------------------------
   [ (eq? word 'iff) #t]
   [ (eq? word 'xor) #t]
   ;Otherwise
...
(define (implication? expr)
  (and (list? expr)
       (= (length expr) 3) ; ( implication op1 op2 )
       (eq? (first expr) 'implies))) ; key word is implies
;---------------------------------------------------------------------------------------------------------
...
(define (get-variables expr)
  (cond
    [(constant? expr) '()] ; no variables here!
    [(variable? expr) (list expr)] ; return that expr!

    ; need to use union to remove duplicates!
    [(or (disjunction? expr) (conjunction? expr) (implication? expr))
         (union (get-variables (op1 expr)) (get-variables (op2 expr)))] ; if an operator, look at the two operands for variables
    ; ignore if user types in nonsense
    ; 1 vs 2 operands
...
(define (generate-truth-table vars)
  (cond
    [(null? vars) '(())] ; nothing in list, return '(()) so we can map stuff later
    ; if we return '() we can't use map
    
    ; recursive defn:
    ;   empty vars = '(())
    ;   else = ( (a #t) (truth table for rest) ) ( (a #f) (truth table for rest) )
    ;   need to cons (a #t) and (a #f) to all elements in the rest of the truth table list so we use map
    [else (append (map (lambda (x) (cons (binding (first vars) #t) x)) (generate-truth-table (rest vars)))
                  (map (lambda (x) (cons (binding (first vars) #f) x)) (generate-truth-table (rest vars))))]
    )
  )
;---------------------------------------------------------------------------------------------------------
...
;---------------------------------------------------------------------------------------------------------
(define (special-and a b)
  (and a b)
  )

(define (is-tautology? expr)
  (foldr special-and #t ; and all the generated boolean values
         (map ; apply boolean-eval to the expr using all the env in the truth table
          (lambda (x) (boolean-eval expr x)) 
          (generate-truth-table (get-variables expr))) ; the truth table 
         )
  )
;---------------------------------------------------------------------------------------------------------
