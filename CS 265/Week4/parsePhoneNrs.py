...
#
# Modified for CS265 Lab 4 by Geoffrey Xiao, gx26
# 		Able to handle extensions
...
def searchFile( fileName, pattern ) :
	...
	for l in fh :
		...
		if match :
			...
			# Handling the existence of the extension
			if not nr[3] :
				ext = DEF_A_CODE
			else :
				ext = nr[3]	
...
def main() :
	...
	patString = r'''
		...
		[ .\-]*			# optional separator (any # of space, dash, or dot)
		(\d{4})			# number, 4 digits
		[ .\-x]*		# Optional seperator


########## Regex for the extension #########
		([0-9]{1,4})?		# Extension, optional, 1 to 4 numbers
		'''
